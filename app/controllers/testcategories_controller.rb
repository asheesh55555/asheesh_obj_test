class TestcategoriesController < ApplicationController
  ...
  def create
    @testcategory = Testcategory.new(testcategory_params)
     @testcategory.user_id = current_user.id
  ...
  private
    ...
    def testcategory_params
      params.require(:testcategory).permit(:testcategoryname, :testname ,tests_attributes: [:id, :testname, :_destroy,   :qname ,questions_attributes: [:id, :qname, :_destroy, :aname ,answers_attributes: [:id, :aname, :_destroy]] ])
    end
end
